...
model = tf.keras.models.Sequential([
    ...
    tf.keras.layers.Dense(256, activation='relu'),
    tf.keras.layers.Dropout(0.2),
    tf.keras.layers.Dense(len(training_images.class_indices), activation='softmax')
])
...
hist = model.fit(
    training_images,
    epochs=epochs,
    callbacks=[model_check, earlystop],
    verbose=1,
...
# Salvar histórico
pd.DataFrame(hist.history).to_csv('historico_treinamento.csv', index=False)
